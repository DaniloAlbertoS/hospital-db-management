...
![Diagrama de Entidade Relacionamento](./diagrama_hospital_db_danilo_alberto.jpg)
...
1. **Criação (DDL):** Scripts de criação de tabelas, definições de chaves primárias (PK), chaves estrangeiras (FK) e evoluções de esquema via `ALTER TABLE`.
2. **Povoamento (DML):** Scripts de inserção de dados fictícios para testes de massa e validação de relacionamentos.
3. **Consultas Avançadas (DQL):** Relatórios complexos desenvolvidos para análise de dados:
